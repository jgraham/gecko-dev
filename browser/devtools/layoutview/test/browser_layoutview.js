...
let test = asyncTest(function*() {
  ...
  yield testInitialValues(view);
  yield testUpdatedValues(view, node, inspector);
});

function* testInitialValues({doc}) {
...
function* testUpdatedValues({doc}, node, inspector) {
  info("Test that changing the document updates the box model");

  info("Updating the node's style");
  let onUpdated = inspector.once("inspector-updated");
  node.style.height = "150px";
  node.style.paddingRight = "50px";
  yield onUpdated;

  info("Checking that the box model values have updated");
